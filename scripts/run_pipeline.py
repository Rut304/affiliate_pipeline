...
from _utils import env_run_id, ensure_dir, log
...
def collect_summary() -> None:
        ...
        if os.path.isdir(packs_dir):
            for pack_dir in sorted(os.listdir(packs_dir)):
                rows.append({
                    "run_id": RUN_ID,
                    "pack": pack_dir,
                    "input_yaml_found": cta_by_pack.get(pack_dir, {}).get("input_yaml_found",""),
    ...
    with open(out, "w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "run_id","pack","input_yaml_found","cta_changed","cta_note",
            "narration_count","images_matched","fallbacks_created","images_missing_after"
        ])
        w.writeheader()
        for r in rows:
            w.writerow(r)
