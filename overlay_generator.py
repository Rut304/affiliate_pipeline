...
def generate_overlays(narration_dir: str, out_dir: str, style_name: str, require_cta=True) -> list[Path]:
    ...
    for txt in sorted(n_dir.glob("*.txt")):
        ...
        if require_cta and "[CTA_PRIMARY]" not in text:
            print(f"[skip] {txt.name}: missing [CTA_PRIMARY] (preflight should prevent this)")
            continue
...
def main():
    ap = argparse.ArgumentParser(description="Generate overlay JSON specs from narration files.")
    ...
    ap.add_argument("--allow-missing-cta", action="store_true", help="Generate specs even if CTA placeholder is missing")
    args = ap.parse_args()
