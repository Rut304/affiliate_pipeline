...
def validate_matching(images_folder="images", narration_folder="narration", auto_stub=True, auto_cleanup=True):
    image_files = [f for f in os.listdir(images_folder) if f.lower().endswith((".jpg", ".png", ".jpeg"))]
    narration_files = [f for f in os.listdir(narration_folder) if f.lower().endswith(".txt")]
    ...
    if missing_narration:
        messages.append(f"⚠️ Missing narration for: {', '.join(sorted(missing_narration))}")
        if auto_stub:
            ...
            messages.append(f"📝 Auto-generated {len(missing_narration)} narration stub(s).")

    if extra_narration:
        messages.append(f"⚠️ Narration without matching image: {', '.join(sorted(extra_narration))}")
        if auto_cleanup:
            for name in extra_narration:
                path = os.path.join(narration_folder, f"{name}.txt")
                os.remove(path)
            messages.append(f"🗑️ Deleted {len(extra_narration)} unmatched narration file(s).")
